...
loop(Morphology,HiddenLayerDensities,MA,EL,FT,BenchmarkIndex,FitnessAcc,EvalsAcc,CyclesAcc,TimeAcc)->
    ...
    Ref = monitor(process, Trainer_PId),
    receive
        ...
        {Trainer_PId, error, Reason} ->
            exit({training_failed, Reason});
        {'DOWN', Ref, process, Trainer_PId, Reason} ->
            exit({training_failed, Reason});
        terminate ->
            exit(Trainer_PId, kill),
